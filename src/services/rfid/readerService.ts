...
export async function approveReader(readerId: string, schoolId: string, actorId?: string) {
  return withTenantContext(schoolId, async (tx) => {
    ...
    await createAuditLog({ schoolId, actorId: sanitizeActorId(actorId), action: 'RFID_READER_APPROVED', resourceId: readerId, resourceType: 'RFID_READER' }, tx);
...
export async function suspendReader(readerId: string, schoolId: string, reason: string, actorId?: string) {
  return withTenantContext(schoolId, async (tx) => {
    ...
    if (reader) {
      await createAuditLog({ schoolId, actorId: sanitizeActorId(actorId), action: 'RFID_READER_SUSPENDED', resourceId: readerId, resourceType: 'RFID_READER', metadata: { reason } }, tx);
    }
    return sanitizeReader(reader);
  });
}
...
export async function revokeReader(readerId: string, schoolId: string, reason: string, actorId?: string) {
  return withTenantContext(schoolId, async (tx) => {
    ...
    if (reader) {
      await createAuditLog({ schoolId, actorId: sanitizeActorId(actorId), action: 'RFID_READER_REVOKED', resourceId: readerId, resourceType: 'RFID_READER', metadata: { reason } }, tx);
    }
    return sanitizeReader(reader);
  });
}
...
export async function getReaderHealth(readerId: string, schoolId: string) {
  return await getReaderById(readerId, schoolId);
}
...
export async function listReaders(schoolId: string, filters?: { status?: 'PENDING' | 'ACTIVE' | 'SUSPENDED' | 'REVOKED' | 'RETIRED' }) {
  return withTenantContext(schoolId, async (tx) => {
    ...
    const readers = await tx.select().from(rfidReaders).where(and(...conditions));
