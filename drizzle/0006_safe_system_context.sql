-- Corrective migration for the 0005 SYSTEM sentinel regression.
-- app.current_school_id is always empty or a UUID; system access is carried
-- by the separate transaction-local app.is_system flag.
DO $$
DECLARE
  table_name text;
...
BEGIN
  ...
  -- Auth tables are reachable only from withSystemContext. Tenant services
  -- continue to join users normally, so users/schools retain their existing
  -- deployment behavior while auth_sessions is explicitly protected.
