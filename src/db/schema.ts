...
export const rfidCredentials = pgTable(
  'rfid_credentials',
  {
    ...
    schoolId: uuid('school_id').notNull().references(() => schools.id, { onDelete: 'cascade' }),
    studentId: uuid('student_id').notNull().references(() => students.id, { onDelete: 'cascade' }),
    credentialType: varchar('credential_type', { length: 50 }).notNull().default('UHF_EPC_GEN2'),
    credentialDigest: varchar('credential_digest', { length: 255 }).notNull(),
    epcLastFour: varchar('epc_last_four', { length: 8 }),
    tidDigest: varchar('tid_digest', { length: 255 }),
...
export const rfidReaders = pgTable(
  'rfid_readers',
  {
    ...
    readerModel: varchar('reader_model', { length: 100 }).default('ZEBRA_FX9600'),
    firmwareVersion: varchar('firmware_version', { length: 100 }),
    adapterType: rfidAdapterTypeEnum('adapter_type').notNull().default('NETWORK'),
    securityCapability: varchar('security_capability', { length: 100 }).notNull().default('ZEBRA_FX9600'),
    assignedClassSectionId: uuid('assigned_class_section_id').references(() => classSections.id, { onDelete: 'set null' }),
    antennaConfig: jsonb('antenna_config'),
    bearerTokenDigest: varchar('bearer_token_digest', { length: 255 }),
...
export const rfidScanEvents = pgTable(
  'rfid_scan_events',
  {
    ...
    epcDigest: varchar('epc_digest', { length: 255 }),
    epcLastFour: varchar('epc_last_four', { length: 8 }),
    tidDigest: varchar('tid_digest', { length: 255 }),
    antennaPort: integer('antenna_port'),
    peakRssi: integer('peak_rssi'),
    readCount: integer('read_count').default(1),
    ...
    captureMethod: captureMethodEnum('capture_method').notNull().default('RFID_GATE'),
    securityMode: rfidSecurityModeEnum('security_mode').notNull().default('UHF_EPC'),
