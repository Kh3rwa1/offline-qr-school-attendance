...
const toastIcons: Record<ToastKind, React.ReactNode> = {
  success: <CheckCircle2 className="w-5 h-5 text-success-600 shrink-0" strokeWidth={2} />,
  warning: <AlertTriangle className="w-5 h-5 text-warning-600 shrink-0" strokeWidth={2} />,
  error: <XCircle className="w-5 h-5 text-danger-600 shrink-0" strokeWidth={2} />,
  info: <Info className="w-5 h-5 text-info-600 shrink-0" strokeWidth={2} />,
};
...
export const Toast: React.FC<ToastProps> = ({
  ...
  return (
    <motion.div
      ...
      role="status"
      aria-live="polite"
      className={`rounded-2xl p-4 text-sm font-semibold shadow-md flex items-center justify-between gap-3 border ${toastStyles[kind]} ${className}`}
    >
      ...
      <div className="flex items-center gap-2 shrink-0">
        {actionText && onAction && (
          <button
            type="button"
            onClick={onAction}
            className="text-xs font-bold underline px-2 py-1 rounded hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer"
          >
            {actionText}
          </button>
        )}
