...
describe('10/10 Application Security Hardening & Regression Suite', () => {
  ...
  it('proves production startup fails closed when CSRF_SECRET/SESSION_SECRET is missing or weak (< 32 chars)', () => {
    const oldEnv = { ...process.env };
    try {
      process.env.NODE_ENV = 'production';
      process.env.COMPONENT = 'web';
      process.env.SESSION_SECRET = '';
      process.env.CSRF_SECRET = 'weak';
      process.env.DATABASE_URL = 'postgres://user:pass@localhost:5432/db';
      process.env.REDIS_URL = 'redis://localhost:6379';
      process.env.REDIS_KEY_HMAC_SECRET = 'ci-redis-hmac-secret-012345678901234567890123456789';
      process.env.RFID_HMAC_SECRET = 'ci-rfid-hmac-secret-012345678901234567890123456789';
  ...
  it('proves production startup fails closed when ALLOW_TEST_BYPASS is set to true', () => {
    const oldEnv = { ...process.env };
    try {
      process.env.NODE_ENV = 'production';
      process.env.COMPONENT = 'web';
