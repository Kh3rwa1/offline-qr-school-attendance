...
import { RfidOperatorDashboard } from './components/dashboards/RfidOperatorDashboard';
import { ReportViewerDashboard } from './components/dashboards/ReportViewerDashboard';
...
export default function App() {
  ...
  const [activeView, setActiveView] = useState<'scanner' | 'roster' | 'review' | 'reports' | 'admin' | 'rfid' | 'super-admin' | 'school-admin' | 'teacher' | 'rfid-operator' | 'report-viewer'>('scanner');
      ...
      <div className="max-w-7xl mx-auto space-y-5">
        <header className="bg-white rounded-3xl p-5 shadow-sm flex flex-wrap gap-4 items-center justify-between">
          ...
          <nav className="w-full flex flex-wrap gap-2 border-t pt-3">
            ...
            {activeMembership?.role === 'SUPER_ADMIN' && <button onClick={() => setActiveView('super-admin')} className={`px-3 py-2 rounded-lg text-sm font-bold ${activeView === 'super-admin' ? 'bg-purple-50 text-purple-700' : ''}`}>Super Admin</button>}
            {admin && <button onClick={() => setActiveView('school-admin')} className={`px-3 py-2 rounded-lg text-sm font-bold ${activeView === 'school-admin' ? 'bg-blue-50 text-blue-700' : ''}`}>School Admin</button>}
            <button onClick={() => setActiveView('teacher')} className={`px-3 py-2 rounded-lg text-sm font-bold ${activeView === 'teacher' ? 'bg-emerald-50 text-emerald-700' : ''}`}>Teacher</button>
            ...
            <button onClick={() => setActiveView('report-viewer')} className={`px-3 py-2 rounded-lg text-sm font-bold ${activeView === 'report-viewer' ? 'bg-cyan-50 text-cyan-700' : ''}`}>Analytics</button>
        ...
        {activeView === 'admin' && admin && <section className="bg-white rounded-3xl p-5 shadow-sm"><h2 className="text-xl font-black mb-4">Credential administration</h2><p className="text-sm text-slate-500 mb-4">Reissue explicitly revokes the previous credential before issuing a new one.</p><div className="grid md:grid-cols-2 gap-3">{roster.map((student) => <div key={student.studentId} className="border rounded-xl p-3 flex justify-between items-center"><span><b>{student.name}</b><small className="block text-slate-500">Roll {student.rollNumber}</small></span><button onClick={() => void reissueQr(student.studentId)} className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-xs font-bold"><ShieldAlert className="inline w-4 mr-1" />Reissue</button></div>)}</div></section>}
            ...
            {activeView === 'school-admin' && (
              <section className="space-y-4">
                <SchoolAdminDashboard students={[]} schoolId={auth.schoolId} onReissueQr={reissueQr} onRevokeQr={() => {}} />
              </section>
            )}
            ...
            {activeView === 'rfid-operator' && (
              <section className="space-y-4">
                <RfidOperatorDashboard students={[]} schoolId={auth.schoolId} />
              </section>
            )}

            {activeView === 'report-viewer' && (
              <section className="space-y-4">
                <ReportViewerDashboard students={[]} />
              </section>
            )}
      </div>
